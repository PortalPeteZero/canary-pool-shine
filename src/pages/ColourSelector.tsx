        ...
        <div className="container">
          <Tabs defaultValue={finishData.families[0].id} className="w-full">
            <div className="mb-8 overflow-x-auto pb-2">
              <TabsList className="inline-flex h-auto p-1 bg-secondary rounded-lg">
                {finishData.families.map((family) => (
                  <TabsTrigger
                    key={family.id}
                    value={family.id}
                    className="px-4 py-3 text-sm font-semibold uppercase tracking-wide rounded data-[state=active]:bg-card data-[state=active]:shadow-card data-[state=active]:text-primary whitespace-nowrap"
                  >
                    {family.name}
                  </TabsTrigger>
                ))}
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {familyFinishes.map((finish) => (
                      <button
                        ...
                        className="group text-left p-4 rounded-lg bg-card border border-border shadow-card hover:shadow-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
